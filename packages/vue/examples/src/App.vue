...
import { ref } from 'vue';
import { useExporter, useWorkerExporter } from '@bsg-export/vue';
  ...
  <div class="app">
    ...
    <section class="section">
      ...
      <table id="demo-table">
        ...
        <tbody>
          <tr v-for="row in sampleData" :key="row.name">
            <td>{{ row.name }}</td>
            <td>{{ row.age }}</td>
            <td>{{ row.city }}</td>
            <td>{{ row.email }}</td>
      ...
      <div class="actions">
        <button
          ...
          @click="handleTableExport(ExportFormat.Csv)"
        >
          导出 CSV
        </button>
        <button
          :disabled="!initialized || loading"
          @click="handleTableExport(ExportFormat.Xlsx)"
        >
          导出 Excel
        </button>
      </div>
      <div v-if="loading" class="progress">导出中... {{ progress }}%</div>
    </section>
    ...
    <section class="section">
      ...
      <div class="actions">
        <button
          :disabled="!initialized || loading"
          @click="handleDataExport(ExportFormat.Csv)"
        >
          导出 CSV
        </button>
        <button
          :disabled="!initialized || loading"
          @click="handleDataExport(ExportFormat.Xlsx)"
        >
          导出 Excel
        </button>
      </div>
    ...
    <section class="section">
      ...
      <div class="actions">
        <button
          ...
          @click="handleWorkerExport(ExportFormat.Csv)"
        >
          Worker 导出 CSV
        </button>
        <button
          :disabled="!workerReady || workerLoading"
          @click="handleWorkerExport(ExportFormat.Xlsx)"
        >
          Worker 导出 Excel
        </button>
      </div>
      <div v-if="workerLoading" class="progress">
