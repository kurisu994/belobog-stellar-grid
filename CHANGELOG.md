...
- 🚀 Excel 预览隐藏行列不渲染：自动检测 `hidden="1"` 的行和列并跳过，合并单元格跨度自动扣减隐藏行列

### 修复 (Fixed)

- 🐛 修复自闭合 `<xf ... />` 元素在样式解析时被丢弃，导致后续所有样式索引偏移的问题
- 🐛 修复 `parse_sheet_xml` 重构后遗漏 `c`/`col` 元素的 Start 分支处理，导致所有单元格样式和格式化失效
- 🐛 修复隐藏列中的合并单元格原点不可见时，非原点单元格仍被 skip_set 跳过，导致后续列整体左移错位
...
- 🐛 修复 Excel 预览样式缺失与文字不换行问题：
  - 将所有预览单元格设置为最大宽度 200px（约 16 个中文字符）并自动换行，避免长内容撑爆表格
  - 移除前端样式和 WASM 默认样式中限制文字换行的 `white-space:nowrap`
  - 为缺乏内置样式描述的 `.xls` 格式补充了边框、字号、内边距等补偿样式
  - 优化 `wrapText` 解析，确保即使外部 CSS 存在冲突也能通过内联样式保证渲染效果
