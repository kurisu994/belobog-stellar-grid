...
├── core/
│   ├── mod.rs          # 统一 API (export_table, export_data, export_tables_xlsx, parseExcelToHtml/Json)
│   ├── style.rs        # Excel 导出样式模块 (三级样式体系：全局→列级→单元格)
...
├── streaming_export.rs # 流式 CSV 数据导出 (分块写入 + Blob 拼接)
└── utils.rs            # 辅助工具函数
...
├── svelte/             # @bsg-export/svelte — Svelte Store 封装
...
just build-packages                 # 构建子包 (types/react/vue/svelte/solid/worker)
just publish-packages               # 发布子包到 npm
```

## CHANGELOG 规范

- **已发布版本限制**：有版本号的部分是已经发布的版本，**禁止**往里面添加新的变更日志。
- **新变更位置**：所有尚未发布的新增、修复、优化等变更日志，必须统一添加到 `## [Unreleased]` 栏目下。
...
cargo test --test test_security         # 安全测试
cargo test --test test_streaming_export # 流式导出逻辑测试
cargo test --test test_excel_preview    # Excel 预览解析测试
...
- **样式系统**: `src/core/style.rs` - 三级样式体系
- **验证**: `src/validation.rs` - 安全检查
- **CSV 导出**: `src/core/export_csv.rs` - 格式生成
- **XLSX 导出**: `src/core/export_xlsx.rs` - Excel 生成
- **Excel 解析**: `src/core/excel_reader.rs` - calamine 解析核心
- **Excel 样式**: `src/core/excel_style.rs` - OOXML → CSS 映射
- **HTML 构建**: `src/core/html_builder.rs` - HTML Table 拼装
- **分批处理**: `src/batch_export.rs` 和 `src/batch_export_xlsx.rs`
- **流式导出**: `src/streaming_export.rs` - 分块写入
