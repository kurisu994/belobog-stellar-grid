...
| **1.1.8** | 2026-03-25 | 预览条件格式渲染；隐藏行列不渲染；自闭合 `xf` / `parse_sheet_xml` 遗漏修复 |
| **1.1.7** | 2026-03-23 | Svelte / Solid 预览组件；批量 XLSX 自动计算表头行数；`apply_tint` 溢出修复 |
| **1.1.6** | 2026-03-20 | 预览样式缺失与文字不换行修复（max-width、white-space、xls 补偿样式） |
| **1.1.5** | 2026-03-20 | 隐藏工作表过滤（calamine）；xls 格式兼容（`open_workbook_auto_from_rs`）；xls 预览降级 |
| **1.1.4** | 2026-03-20 | 数字格式动态解析；隐藏 Sheet 过滤（workbook.xml）；预览样式对齐 Excel |
| **1.1.3** | 2026-03-20 | （版本占位，无内容） |
| **1.1.2** | 2026-03-20 | Excel 在线预览基础能力（calamine 解析 + HTML/JSON 双输出）；React/Vue 预览组件与类型 |
| **1.1.1** | 2026-03-19 | 三级样式体系（全局 → 列级 → 单元格级）；性能基准测试体系；style-export 示例 |
| **1.1.0** | 2026-03-03 | 流式 CSV 导出；5 个框架示例项目；Worker 初始化与 StrictMode 竞态修复 |
| **1.0.x** | 2025-12 ~ 2026-02 | 基础功能建立：HTML 表格导出、分批异步、多 Sheet、数据/树形导出、冻结窗格、文件名校验、RAII |

## 重大架构变更

### 1. 全量代码审查与健壮性修复（2026-08-18 / 提交 `32c5361`）
