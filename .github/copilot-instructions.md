...
Rust WebAssembly 库，在浏览器端将 HTML 表格或 JS 数据导出为 CSV/XLSX 文件，并支持 Excel 在线预览。
...
- **预览模式** (`parseExcelToHtml/Json`): `excel_reader.rs` 解析 Excel → `html_builder.rs` 生成 HTML 或 JSON

关键模块：
- `src/core/mod.rs` — 统一入口：`export_table`、`export_data`、`export_tables_xlsx`、`parseExcelToHtml`、`parseExcelToJson`、`getExcelSheetList`
