...
#### 预览模式 (parseExcelToHtml / parseExcelToJson)

```
Uint8Array → excel_reader.rs (calamine + zip 解析)
           → excel_style.rs (OOXML 样式 → CSS 映射)
           → html_builder.rs (HTML Table 生成) 或 serde_json (JSON 输出)
```
...
├── core/               # 核心业务逻辑
│   ├── mod.rs          # 统一 API (export_table, export_data, export_tables_xlsx, generate_data_bytes, parseExcelToHtml/Json)
│   ├── style.rs        # Excel 导出样式模块 (三级样式体系：全局→列级→单元格，CellStyle/StyleSheet)
...
#### Excel 预览 (src/core/excel_reader.rs + excel_style.rs + html_builder.rs)
...
- **Excel 预览功能**: 添加到 `test_excel_preview.rs`
