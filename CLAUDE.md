...
- **运行特定测试**: `cargo test --test lib_tests` (基础功能测试), `cargo test --test test_resource` (RAII 资源测试), `cargo test --test test_unified_api` (统一 API 测试)
...
- **函数**: `export_table`
- **功能**: 支持 CSV 和 XLSX 格式导出，支持进度回调、隐藏行列排除、合并单元格保留
- **参数**: table_id, filename, format, exclude_hidden, progress_callback
- **特点**: 智能格式检测，自动添加文件扩展名
